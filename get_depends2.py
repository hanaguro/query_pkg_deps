...
返すファイル)について，readelf -dした結果をデータベース ./depends2.sql3 に
記録する．root権限でしか読めないファイルもあるので，sudo で実行することが望ましい．

./depends2.sql3 には depends というテーブルがあり，そのテーブルの構造は
...
データベースを更新するには古いファイル(./depends2.sql3)を削除して，
再度このコマンドを実行すること．
...
import sqlite3, os, subprocess, sys, time, magic, re
...
def extract_needed_libs(text: str):
    # 「共有ライブラリ: [xxx]」の xxx をすべて取得
    pattern = r"共有ライブラリ:\s*\[([^\]]+)\]"
...
def get_depends(file):
    list = []
    try:
        res = subprocess.check_output(['readelf', '-d', file]).decode('utf-8')
        tmp = extract_needed_libs(res)
        for i in tmp:
            list.append(i.lstrip())

    except subprocess.CalledProcessError:
        print(("error occured to readelf {0}. maybe different archs?".format(file)))
